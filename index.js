...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
//The nested function has access to the outside scope due to closures.  It therfore can access the variable declared as internal.  
...
function animalNames(arr) {
  return arr.map(i => `name: ${i.animal_name}, scientific: ${i.scientific_name}`);
}
...
function USApop(arr) {
  return arr.reduce((total, eachAnimal) => {
    return total + eachAnimal.population;
  }, 0);
}
...
*/
function CuboidMaker(ats) {
  this.length = ats.length;
  this.width = ats.width;
  this.height = ats.height;
}
...
//🦄🦄🦄 Test your volume and surfaceArea methods by uncommenting the logs below: 🦄🦄🦄
console.log(cuboidTwo.volume(), "::volume from class"); // 100
console.log(cuboidTwo.surfaceArea(), "::surface are from class"); // 130
